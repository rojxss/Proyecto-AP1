  ...
  return (
    <section className="tablon" aria-label="Tablón de avisos">
      ...
      <style>{`
        ...
        .avisos-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(230px,1fr)); gap: 1rem; align-items: stretch; }
        .aviso-nota { background: var(--blanco); border: 1px solid var(--linea); border-radius: 10px; padding: 1rem; position: relative; box-shadow: var(--sombra); transition: transform 0.4s cubic-bezier(.34,1.56,.64,1), box-shadow 0.4s ease; display: flex; flex-direction: column; }
        ...
        .aviso-nota p { font-size: .88rem; color: var(--tinta-suave); flex: 1; }
      `}</style>
    </section>
  )
}
